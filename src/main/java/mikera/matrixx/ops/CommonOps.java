...
public class CommonOps {
   ...
   * c = &alpha; * a * b <br>
   * <br>
   * c<sub>ij</sub> = &alpha; &sum;<sub>k=1:n</sub> { * a<sub>ik</sub> *
   * b<sub>kj</sub>}
   * </p>
   * 
   ...
   * <br>
   * c<sub>ij</sub> = &alpha; &sum;<sub>k=1:n</sub> { a<sub>ki</sub> *
   * b<sub>kj</sub>}
   * </p>
   * 
   * @param alpha Scaling factor.
   ...
   * 
   * <p>
   * Is faster than using a generic matrix multiplication by taking advantage of
   * symmetry. For vectors there is an even faster option, see
   * {@link mikera.matrixx.algo.mult.VectorVectorMult#innerProd(Matrix, Matrix)}
   * </p>
   * 
  ...
  public static void multInner(Matrix a, Matrix c) {
    if (a.columnCount() != c.columnCount() || a.columnCount() != c.rowCount())
      throw new IllegalArgumentException(
          "Rows and columns of 'c' must be the same as the columns in 'a'");
  ...
  public static void multOuter(Matrix a, Matrix c) {
    if (a.rowCount() != c.columnCount() || a.rowCount() != c.rowCount())
      throw new IllegalArgumentException(
          "Rows and columns of 'c' must be the same as the rows in 'a'");
   ...
   * <br>
   * c = c + a * b<br>
   * c<sub>ij</sub> = c<sub>ij</sub> + &sum;<sub>k=1:n</sub> { a<sub>ik</sub> *
   * b<sub>kj</sub>}
   * </p>
   * 
   * @param a The left matrix in the multiplication operation. Not modified.
   ...
   * <br>
   * c = c + &alpha; * a * b<br>
   * c<sub>ij</sub> = c<sub>ij</sub> + &alpha; * &sum;<sub>k=1:n</sub> {
   * a<sub>ik</sub> * b<sub>kj</sub>}
   * </p>
   * 
   * @param alpha scaling factor.
   * @param a The left matrix in the multiplication operation. Not modified.
   ...
   * c<sub>ij</sub> = c<sub>ij</sub> + &sum;<sub>k=1:n</sub> { a<sub>ki</sub> *
   * b<sub>kj</sub>}
   * </p>
   * 
   * @param a The left matrix in the multiplication operation. Not modified.
   ...
   * 
   * <p>
   * If the system could not be solved then false is returned. If it returns
   * true that just means the algorithm finished operating, but the results
   * could still be bad because 'A' is singular or nearly singular.
   * </p>
   * 
   * <p>
   ...
   * @param A_tran Where the transpose is stored. If null a new matrix is
   *          created. Modified.
  ...
  public static Matrix transpose(Matrix A, Matrix A_tran) {
    if (A_tran == null) {
      ...
      if (A.rowCount() != A_tran.columnCount()
          || A.columnCount() != A_tran.rowCount()) {
   ...
   * needed, then using {@link AltLU} directly (or any similar algorithm) can be
   * more efficient.
   * 
   ...
   * the results in the same matrix.<br>
   * <br>
   * a = a<sup>-1<sup>
   * </p>
   * 
   * <p>
   * If the algorithm could not invert the matrix then false is returned. If it
   * returns true that just means the algorithm finished. The results could
   * still be bad because the matrix is singular or nearly singular.
   * </p>
   * 
   * @param mat The matrix that is to be inverted. Results are stored here.
   *          Modified.
   * @return true if it could invert the matrix false if it could not.
   */
  public static boolean invert(Matrix mat) {
    if (mat.columnCount() <= UnrolledInverseFromMinor.MAX) {
      ...
      AltLU alg = new AltLU();
      LULinearSolver solver = new LULinearSolver(alg);
      if (solver.setA(mat)) {
        solver.invert(mat);
      } else {
        return false;
   ...
   * stores the results in another matrix. The two matrices must have the same
   * dimension.<br>
   * <br>
   * b = a<sup>-1<sup>
   * </p>
   ...
   * 
   * <p>
   * For medium to large matrices there might be a slight performance boost to
   * using {@link LinearSolverFactory} instead.
   * </p>
   * 
   ...
   * </p>
   * <p>
   * s<sub>i-y0 , j-x0</sub> = o<sub>ij</sub> for all y0 &le; i < y1 and x0 &le;
   * j < x1 <br>
   * <br>
   * where 's<sub>ij</sub>' is an element in the submatrix and 'o<sub>ij</sub>'
   ...
   */
  public static void extract(Matrix src, int srcY0, int srcY1, int srcX0,
      int srcX1, Matrix dst, int dstY0, int dstX0) {
    if (srcY1 < srcY0 || srcY0 < 0 || srcY1 > src.rowCount())
      throw new IllegalArgumentException(
          "srcY1 < srcY0 || srcY0 < 0 || srcY1 > src.rowCount()");
    if (srcX1 < srcX0 || srcX0 < 0 || srcX1 > src.columnCount())
      throw new IllegalArgumentException(
          "srcX1 < srcX0 || srcX0 < 0 || srcX1 > src.columnCount()");
   ...
   */
  public static Matrix extract(Matrix src, int srcY0, int srcY1, int srcX0,
      int srcX1) {
    if (srcY1 <= srcY0 || srcY0 < 0 || srcY1 > src.rowCount())
      throw new IllegalArgumentException(
          "srcY1 <= srcY0 || srcY0 < 0 || srcY1 > src.rowCount()");
   ...
   * <p>
   * 
   * @param src Matrix whose diagonal elements are being extracted. Not
   *          modified.
  ...
  /**
   * Inserts matrix 'src' into matrix 'dest' with the (0,0) of src at (row,col)
   * in dest. This is equivalent to calling
  ...
  public static void scale(double alpha, Matrix a) {
    // on very small matrices (2 by 2) the call to elementCount() can slow it
    // down
    // slightly compared to other libraries since it involves an extra
    // multiplication.
    final long size = a.elementCount();
  ...
  /**
   * <p>
   * Puts the augmented system matrix into reduced row echelon form (RREF). A
   * matrix is said to be in RREF is the following conditions are true:
   * </p>
   * 
   * <ol>
   * <li>If a row has non-zero entries, then the first non-zero entry is 1. This
   * is known as the leading one.</li>
   * <li>If a column contains a leading one then all other entries in that
   * column are zero.</li>
   * <li>If a row contains a leading 1, then each row above contains a leading 1
   * further to the left.</li>
   * </ol>
   * 
   * <p>
   * [1] Page 19 in, Otter Bretscherm "Linear Algebra with Applications"
   * Prentice-Hall Inc, 1997
   * </p>
   * 
   * @param A Input matrix. Unmodified.
   * @param numUnknowns Number of unknowns/columns that are reduced. Set to -1
   *          to default to Math.min(A.rowCount(),A.columnCount()), which works
   *          for most systems.
   * @param reduced Storage for reduced echelon matrix. If null then a new
   *          matrix is returned. Modified.
  ...
  public static Matrix rref(Matrix A, int numUnknowns, Matrix reduced) {
    ...
    } else if (reduced.columnCount() != A.columnCount()
        || reduced.rowCount() != A.rowCount())
      throw new IllegalArgumentException(
          "'re' must have the same shape as the original input matrix");
