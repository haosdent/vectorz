...
/**
 * <p>
 * QR decomposition can be used to solve for systems. However, this is not as
 * computationally efficient as LU decomposition and costs about 3n<sup>2</sup>
 * flops.
 * </p>
 * <p>
 * It solve for x by first multiplying b by the transpose of Q then solving for
 * the result. <br>
 * QRx=b<br>
 * Rx=Q^T b<br>
 * </p>
 ...
 * @author Peter Abeles
 */
public class HouseholderColumnQRLinearSolver extends ALinearSolver {

  protected int maxRows = -1;
  protected int maxCols = -1;
  private HouseholderColumnQR decomposer;
  private Matrix a = Matrix.create(1, 1);
  private Matrix temp = Matrix.create(1, 1);
  private double[][] QR; // a column major QR matrix
  private Matrix R = Matrix.create(1, 1);
  private double gammas[];

  /**
   * Creates a linear solver that uses QR decomposition.
   */
  public HouseholderColumnQRLinearSolver() {
    decomposer = new HouseholderColumnQR();
  }
  ...
  public void solve(Matrix B, Matrix X) {
    if (X.rowCount() != numCols)
      throw new IllegalArgumentException(
          "Unexpected dimensions for X: X rows = " + X.rowCount()
              + " expected = " + numCols);
    ...
    for (int colB = 0; colB < BnumCols; colB++) {
      ...
      for (int n = 0; n < numCols; n++) {
        ...
        double vv = u[n];
        u[n] = 1;
        QrHelperFunctions.rank1UpdateMultR(a, u, gammas[n], 0, n, numRows,
            temp.data);
        u[n] = vv;
      }
