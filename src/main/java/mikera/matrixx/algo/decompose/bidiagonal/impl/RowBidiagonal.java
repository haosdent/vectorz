...
/**
 * <p>
 * Performs a {@link IBidiagonal} using householder reflectors. This is
 * efficient on wide or square matrices.
 * </p>
 * 
 * @author Peter Abeles
 */
public class RowBidiagonal implements IBidiagonal {
   ...
   */
  public RowBidiagonal(int numElements) {

    UBV = Matrix.create(numElements);
  ...
  public RowBidiagonal() {
    this(1);
  }

  public static Matrix handleB(Matrix B, boolean compact, int m, int n, int min) {
    int w = n > m ? min + 1 : min;

    if (compact) {
      if (B == null) {
        B = Matrix.create(min, w);
      } else {
        B.reshape(min, w);
  ...
  public static Matrix handleU(Matrix U, boolean transpose, boolean compact,
      int m, int n, int min) {
    if (compact) {
      if (transpose) {
        if (U == null)
          U = Matrix.create(min, m);
        else {
          U.reshape(min, m);
        }
      } else {
        if (U == null)
          U = Matrix.create(m, min);
        else
          U.reshape(m, min);
      }
    } else {
  ...
  public static Matrix handleV(Matrix V, boolean transpose, boolean compact,
      int m, int n, int min) {
    int w = n > m ? min + 1 : min;
  ...
  /**
   * Computes the decomposition of the provided matrix. If no errors are
   * detected then true is returned, false otherwise.
   * 
   ...
   * 
   * @param B If not null the results are stored here, if null a new matrix is
   *          created.
   ...
   * 
   * @param U If not null then the results will be stored here. Otherwise a new
   *          matrix will be created.
   * @return The extracted Q matrix.
   */
  @Override
  public Matrix getU(Matrix U, boolean transpose, boolean compact) {
   ...
   * 
   * @param V If not null then the results will be stored here. Otherwise a new
   *          matrix will be created.
   * @return The extracted Q matrix.
   */
  @Override
  public Matrix getV(Matrix V, boolean transpose, boolean compact) {
    V = handleV(V, transpose, compact, m, n, min);
    CommonOps.setIdentity(V);

    // UBV.print();

    // todo the very first multiplication can be avoided by setting to the
    // rank1update output
    for (int j = min - 1; j >= 0; j--) {
      u[j + 1] = 1;
      for (int i = j + 2; i < n; i++) {
      ...
      if (transpose)
        ...
        QrHelperFunctions.rank1UpdateMultR(V, u, gammasV[j], j + 1, j + 1, n,
            this.b);
    }
  ...
  protected void computeU(int k) {
    ...
    for (int i = k; i < m; i++) {
      // copy the householder vector to vector outside of the matrix to reduce
      // caching issues
      // big improvement on larger matrices and a relatively small performance
      // hit on small matrices.
