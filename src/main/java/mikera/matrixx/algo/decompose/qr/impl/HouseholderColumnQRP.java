...
/**
 * <p>
 * Performs QR decomposition with column pivoting. To prevent overflow/underflow
 * the whole matrix is normalized by the max value, but columns are not
 * normalized individually any more. To enable code reuse it extends
 * {@link HouseholderColumnQR} and functions from that class are used whenever
 * possible. Columns are transposed into single arrays, which allow for fast
 * pivots.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * Based off the description in "Fundamentals of Matrix Computations", 2nd by
 * David S. Watkins.
 * </p>
 * 
 * @author Peter Abeles
 */
public class HouseholderColumnQRP extends HouseholderColumnQR implements IQRP {
  // the ordering of each column, the current column i is the original column
  // pivots[i]
   ...
   */
  public HouseholderColumnQRP(double singularThreshold) {
    this.singularThreshold = singularThreshold;
  }

  public HouseholderColumnQRP() {
  }
  ...
  /**
   * Sets the initial pivot ordering and compute the F-norm squared for each
   * column
   */
  ...
  private void updateNorms(int j) {
    ...
    // if a negative sum has been found then clearly too much precision has been
    // last
   ...
   * The already computed norm is used and checks to see if the matrix is
  ...
  @Override
  public Matrix getPivotMatrix(Matrix P) {
    if (P == null)
      P = Matrix.create(numCols, numCols);
